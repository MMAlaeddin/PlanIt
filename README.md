...
_Confirm that you have navigated to the PlanIt directory (e.g., by entering the command_ ``pwd`` _in Terminal)._

## Clone the PlanIt repository and run the PlanIt app

_Enter the following commands in Terminal (macOS) or PowerShell (Windows):_
* ``cd ~``
* ``cd desktop``
* ``git clone https://github.com/MMAlaeddin/PlanIt``
* ``cd PlanIt/PlanIt``
* ``dotnet restore``
* ``dotnet build``
